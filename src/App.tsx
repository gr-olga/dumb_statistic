  ...
  useEffect(() => {
    if (userCountryId) {
      getMemoizedCountryData(userCountryId).then((data) => {
        const calculatedLifeExpectancy = calculateLifeExpectancy(
          data as TDemographicData,
        );
      ...
      {start && (
        <div className={styles.App}>
          <div className={styles.right_col}>
            <UserForm />
            <h6>
              Life expectancy in {userData.user.country.location} for{" "}
              {userData.user.sex} is {lifeExpectancy}
            </h6>
            <div>
